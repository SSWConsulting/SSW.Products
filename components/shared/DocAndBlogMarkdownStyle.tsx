...
export const DocAndBlogMarkdownStyle: Components<{}> = {
  ...
  ol: (props) => (
    <ol className="list-decimal font-lightlist-inside pl-6 mb-4">{props?.children}</ol>
  ),

  ul: (props) => (
    <ul className="list-disc font-light list-inside pl-6 mb-4">{props?.children}</ul>
  ),
...
//   strong: (props: { children: string | number | bigint | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | Promise<AwaitedReactNode> | null | undefined; }) => <strong className="font-bold">{props.children}</strong>,
  ...
  img: (props) => (
    <div className="my-6">
      ...
      />
      {props?.caption && (
        <p className="text-sm text-gray-600 text-center mt-2">{props?.caption}</p>
      )}
    </div>
  ),
};
