...
export const CodeBlockSkeleton = ({ hasTabs = false }) => {
    ...
    return (
        <div className={`relative w-full ${hasTabs ? "" : "my-2"}`}>
            ...
            <div
                className={`shiki w-full overflow-x-auto bg-[#2d2d2d] py-4 px-2 text-sm shadow-sm rounded-lg ${hasTabs ? "" : "border border-[#404040]"
                    }`}
            >
                <div className="space-y-2">
                    {skeletonLines.map((line, index) => (
                        <div key={index} className="flex items-center space-x-4">
                            <div className="w-8 h-4 bg-[#404040] rounded animate-pulse flex-shrink-0" />

                            <div className="flex-1 space-y-1">
                                <div
                                    className="h-4 bg-[#404040] rounded animate-pulse"
                                    style={{
                                        width: line.width,
...
const InlineCopyButton = () => {
    return (
        <div className="absolute top-0 right-0 z-10 px-4 py-1 text-xs font-mono text-[#637777]">
            <div className="w-8 h-3 bg-[#404040] rounded animate-pulse" />
        </div>
    );
};
