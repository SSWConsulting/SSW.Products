...
YakShaver 通过 Microsoft Entra ID 连接到您的 Microsoft 365 租户。它是一个多租户云应用，使用管理员同意流程请求 Microsoft Graph 应用权限（组织范围的服务到服务访问）。您的租户管理员只需为组织批准一次访问。YakShaver 不会在您的租户中创建资源。

**适用对象**

* 能够批准访问的租户管理员：全局管理员或云应用管理员
* 终端用户在批准后只需登录 YakShaver，但不授予租户范围的访问权限
...
* 电子邮件地址读取权限 - 访问您的主电子邮件地址和公开电子邮件信息（用于用户识别）
