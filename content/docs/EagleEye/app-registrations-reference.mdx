...
2. During deployment, the Master app creates these three app registrations with prefix **SSW\.EagleEye**:
   * `Engine` (SSW\.EagleEye.Engine)
   * `Api` (SSW\.EagleEye.Api)
   * `Portal` (SSW\.EagleEye.Portal)
3. Then it:
...
* [Delete the Master App After Installation](./delete-master-app)
