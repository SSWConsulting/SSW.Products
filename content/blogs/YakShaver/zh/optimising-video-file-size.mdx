...
summary: "## YakShaver 现支持更大视频文件\n\nYakShaver 现在可以处理大于 25MB\
  ...
  \ 方案：剥离视频画面\n\nYakShaver 不是绕过限制，而是使用 Xabe.FFmpeg 只提取\
...
没有硬性大小限制并将视频分割成 25MB 块进行批量处理，从长远来看并不是好主意。这样做会浪费大量处理能力，且如果没有检查机制，很容易被滥用上传极大文件。
...
我们使用一个实现了 [FFmpeg](https://ffmpeg.org/) 的库，名为 [Xabe.FFmpeg](https://ffmpeg.xabe.net/index.html)，它允许我们操作和处理媒体文件。通过只从视频文件中提取音频，我们显著减小了文件大小，只保留转录服务所需的数据——音频。
