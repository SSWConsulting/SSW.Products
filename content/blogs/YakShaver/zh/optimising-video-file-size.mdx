...
seo:
  ...
  openGraphTitle: YakShaver 文件大小优化 | Lewis Toh
  title: YakShaver 文件大小优化 | Lewis Toh
...
summary: "## YakShaver 现支持更大视频文件\n\nYakShaver 现在可以处理超过 25MB\
  ...
  \ 视频文件效率低且成本高，尤其是分块处理且被滥用时。\n\n***\n\n## 智能解决方案：\
  \ 去除视频画面\n\nYakShaver 不是绕过限制，而是使用 Xabe.FFmpeg 仅提取音频，\
  大幅减小文件大小，同时保持转录质量。\n\n***\n\n## 实际文件大小减少示例\n\n*\
  \ 1 分钟 4K 视频：38.87MB \u279C 0.62MB\n* 30 分钟 2K 视频：209.89MB \u279C 7.95MB\n"
summaryCard: true
title: YakShaver 文件大小优化
---
YakShaver 现在可以处理超过 25MB 的视频！这意味着更长时间和更高质量的视频现在可以通过 YakShaver 处理，从而生成更高质量的工作项或邮件！

#### 为什么最初有 25MB 限制？

我们使用的是 [Azure OpenAI 的 Whisper AI 模型](https://learn.microsoft.com/en-us/azure/ai-services/speech-service/whisper-overview)，该模型目前对文件大小有 25MB 的限制。
...
没有硬性大小限制并将视频分割成 25MB 块进行批处理，从长远来看并不是一个好主意。这会浪费大量处理资源，且如果没有相应的检查机制，极易被滥用上传超大文件。

此外，该功能要求视频文件存储在 Blob 存储中，处理更长视频的成本也会更高。

#### 我们如何突破 25MB 限制？

简而言之，我们没有突破限制，而是去除了 Whisper 不需要的部分——占用视频文件大部分字节的画面内容。

我们使用一个基于代码实现的 [FFmpeg](https://ffmpeg.org/) 库，名为 [Xabe.FFmpeg](https://ffmpeg.xabe.net/index.html)，它允许我们操作和处理媒体文件。通过仅从视频文件中提取音频，我们显著减小了文件大小，只保留转录服务所需的音频数据。
