...
summary: "## YakShaver 现支持更大视频文件\n\nYakShaver 现在可以处理超过 25MB\
  ...
  \ 去除视频画面\n\nYakShaver 不是绕过限制，而是使用 Xabe.FFmpeg 只提取音频，\
  大幅减小文件大小，同时保持转录质量。\n\n***\n\n## 实际文件大小缩减\n\n* 1 分钟\
  \ 4K 视频：38.87MB \u279C 0.62MB \n* 30 分钟 2K 视频：209.89MB \u279C 7.95MB\n"
summaryCard: true
...
没有硬性大小限制并将视频分割成 25MB 块进行批处理，从长远来看并不是好主意。这会浪费大量处理资源，且如果没有限制，很容易被滥用上传极大文件。
...
我们使用了一个基于代码实现 [FFmpeg](https://ffmpeg.org/) 的库，[Xabe.FFmpeg](https://ffmpeg.xabe.net/index.html)，它允许我们操作和处理媒体文件。通过只从视频中提取音频，我们显著减小了文件大小，只保留转录服务所需的数据——音频。
